...
| Non-professional businesses | **Busy, growing businesses with solid growth but no solid financial support network** — contractors, restaurants, auto services, distributors, retail and service operators. **Business savvy but not necessarily financially savvy**: they run everything themselves, have no CFO, and see their CPA once a year at filing time — so growth compounds their exposure (taxes, protection, no succession plan) faster than anyone is watching it | Drop-off package + existing-relationship introductions (Section 9 marketing materials); simple, jargon-free flyers (Eddie's avatar) — respect their business smarts, translate the financial side |
...
```mermaid
flowchart TD
    A[AWARENESS<br/>Social content · Chamber visibility · Speaking · Community events] --> B[TRUST<br/>Workshops · Lead magnets · Case studies · Referral endorsements]
    B --> C[CONVERSION<br/>Book a Planning Session]
    C --> D[4-MEETING SALES CYCLE<br/>Discovery → Landscape → Proposal → Go/No-Go]
    D --> E[CLIENT<br/>Commission · Flat-fee plan · AUM via WAN]
    E --> F[ADVOCACY<br/>Referrals · Testimonials · Community proof]
    F -->|flywheel| A
```
...
flowchart LR
    subgraph Channels
      C1[1. Social Media<br/>FB & IG]
      C2[2. UCLA Anderson<br/>Network]
      C3[3. Toastmasters &<br/>Networking]
      C4[4. Referral Hub<br/>Partners]
      C5[5. Website &<br/>Content]
      C6[6. Chamber of Commerce<br/>ESSENTIAL]
      C7[7. Real Estate<br/>Brokerage Hub]
    end
    C1 --> HUB((Book a<br/>Planning<br/>Session))
    C2 --> HUB
    C3 --> HUB
    C4 --> HUB
    C5 --> HUB
    C6 --> HUB
    C7 --> HUB
    HUB --> CYCLE[4-Meeting<br/>Sales Cycle]
```
...
```mermaid
flowchart LR
    P[Partner refers<br/>business owner] --> S[Planning session<br/>booked]
    S --> M[4-meeting cycle<br/>runs]
    M --> W[Client win:<br/>tax savings + coordination]
    W --> T[We report outcome<br/>back to partner]
    T --> R[Partner trust grows<br/>+ client refers peers]
    R --> P
```
...
flowchart LR
    subgraph Days 1-30
      A1[Join chambers:<br/>Santa Ana + Hispanic]
      A2[Booking page +<br/>Tax Toolkit live]
      A3[Name 50-partner<br/>referral list]
      A4[MARKETING MATERIALS<br/>produced — see list]
    end
    subgraph Days 31-60
      B1[Social cadence<br/>3x/week running]
      B2[First 10 partner<br/>coffees done]
      B3[First chamber<br/>lunch-and-learn]
      B4[Brokerage: 20 agents<br/>trained + equipped]
    end
    subgraph Days 61-90
      C1[First co-branded<br/>CPA workshop]
      C2[Case study #1<br/>published]
      C3[Review KPIs,<br/>double down]
    end
    A1-->B1-->C1
    A2-->B2-->C2
    A3-->B3-->C3
